package com.example.campusdetectionv2.view;
...
public class ZoomImageView extends View{
	 ...
	 * 图片双击状态常量
	 */
	public static final int STATUS_ZOOM_MAX = 5;
	/**
	 * 图片双击状态常量
	 */
	public static final int STATUS_ZOOM_MIN = 6;
	 ...
	 * 用于对图片进行移动和缩放变换的矩阵
	 */
	static final int DOUBLE_CLICK_TIME_SPACE_MIN = 20;
	...
	private int currentStatus2;
	private int doubleclickFinish = 0;
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_DOWN:
			...
			{
				 if (event.getEventTime() - lastClickTime < DOUBLE_CLICK_TIME_SPACE&&event.getEventTime() - lastClickTime > DOUBLE_CLICK_TIME_SPACE_MIN) {  
					   if(totalRatio == 3 *initRatio)
					   {
			        	...
			        	else
			        	{
			        		...
			        		currentStatus2 = STATUS_ZOOM_MAX;
			        	}
			        	//totalTranslateX = event.getX();
			        	//totalTranslateY = event.getY();	
		...
		case MotionEvent.ACTION_MOVE:
			if (event.getPointerCount() == 1) {  
				...
				// 进行缩放倍数检查，最大只允许将图片放大3倍，最小可以缩小到初始化比例
	...
	protected void onDraw(Canvas canvas) {
		...
		if(currentStatus2!=0){
			...
			case STATUS_ZOOM_MIN:
				initBitmap(canvas);
				break;
			case STATUS_ZOOM_MAX:
				zoom(canvas);
				break;
			default:
